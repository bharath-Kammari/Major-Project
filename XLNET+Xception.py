...
from keras.layers import Dense, Input, concatenate, Embedding, GlobalMaxPooling2D
...
xlnet_model = TFXLNetModel.from_pretrained('xlnet-base-cased', return_dict=True)
xlnet_input = Input(shape=(max_len,), dtype='int32')
xlnet_sequence_output = xlnet_model(xlnet_input).last_hidden_state
xlnet_max_pool = GlobalMaxPooling2D()(xlnet_sequence_output)
xlnet_output = Dense(1, activation='sigmoid')(xlnet_max_pool)
xlnet_model = Model(inputs=xlnet_input, outputs=xlnet_output)
...
# Combine xlnet and Xception models with intermediate fusion
print(np.shape(xlnet_model.output))
print(np.shape(xception_model.output))
combined_input = concatenate([xlnet_model.output, xception_model.output])
fusion_output = Dense(num_classes, activation='sigmoid')(combined_input)
fusion_model = Model(inputs=[xlnet_model.input, xception_model.input], outputs=fusion_output)
