...
# Define vgg19_bn model
vgg_input = Input(shape=(img_width, img_height, 3))
vgg_model = VGG19(weights='imagenet', include_top=False, input_tensor=vgg_input)
for layer in vgg_model.layers:
    ...
    if 'block5' in layer.name:
        ...
        if 'batch_normalization' in layer.name:
            layer.trainable = True
vgg_output = GlobalMaxPooling2D()(vgg_model.output)
vgg_output = Dense(num_classes, activation='sigmoid')(vgg_output)
vgg_model = Model(inputs=vgg_input, outputs=vgg_output)

# Combine LSTM and vgg models with intermediate fusion
print(np.shape(lstm_model.output))
print(np.shape(vgg_model.output))
combined_input = concatenate([lstm_model.output, vgg_model.output])
fusion_output = Dense(num_classes, activation='sigmoid')(combined_input)
fusion_model = Model(inputs=[lstm_model.input, vgg_model.input], outputs=fusion_output)
